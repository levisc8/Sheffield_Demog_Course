...
s_z <- function(z, Model) {
  require(mgcv)
  newData <- data.frame(z)
...
P_z1z <- function (z1, z, m.par, survModel) {

    return((1 - p_bz(z, m.par)) * s_z(z, survModel) * G_z1z(z1, z, m.par))
...
mk_K <- function(m, m.par, survModel, L, U) {
